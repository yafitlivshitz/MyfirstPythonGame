...
board_length = 9
boardSize = board_length * board_length
...
# in case the randomized positions are the same, set them to be different
def move_initial_position():
    global bunny_row
    global bunny_col

    bunny_row = boardSize - kid_row
    bunny_col = boardSize - kid_col
...
def init_bunny_location():
    ...
    # todo: set to a spot according to the kid's position
    if new_row == kid_row and new_col == kid_col:
        bunny_row = 0
        bunny_col = board_length
    else:
        bunny_row = new_row
        bunny_col = new_col
